...
export default function HomePage() {
  ...
  const buildPlanZip = useCallback(
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.error || err.message || 'Failed to generate business plan');
    }
  ...
  const generateFullZip = useCallback(
    ...
    if (!genRes.ok) {
      ...
      throw new Error(err.error || err.message || 'Failed to generate plan');
    }
      ...
      for (const doc of folder.docs || []) {
        try {
          ...
          zip
            .folder(bizName)!
            .folder(folderName)!
            .file(`${(doc.filename || doc.title || 'doc').replace(/\s+/g, '_')}.docx`, docxBlob);
        } catch (e) {
  ...
  const runGeneration = useCallback(
    async (productKey: ProductKey, ref?: string) => {
      ...
      try {
        const blob =
          productKey === 'PLAN'
            ? await buildPlanZip(refToUse)
            : await generateFullZip(refToUse);
  ...
  const summary = bizData.name || bizData.industry ? (
      ...
      <div className="relative z-10 flex min-h-screen w-full flex-col items-center justify-center px-4 py-8">
        ...
        {phase === 'chat' && (
          <div className="flex flex-col items-center justify-center gap-6">
            <h1 className="text-3xl font-semibold text-zinc-900 md:text-4xl text-center">
              Good day, Let&apos;s make your business profitable.
            </h1>

            <div className="w-full max-w-2xl rounded-2xl border border-stone-200/60 bg-white/90 shadow-xl backdrop-blur overflow-hidden">
            <div className="max-h-[280px] overflow-y-auto p-4">
              ...
              {loading && (
                <div className="mb-3 mr-8 text-left">
                  <div className="inline-block rounded-xl bg-stone-100 px-4 py-2 text-sm text-stone-600">
                    …
                  </div>
                </div>
              )}
        ...
        {/* Plans phase */}
        {phase === 'plans' && (
          <div className="flex flex-1 flex-col rounded-2xl border border-stone-200/60 bg-white/90 p-6 shadow-xl backdrop-blur">
        ...
        {/* Generating phase */}
        {phase === 'generating' && (
          <div className="flex flex-1 flex-col items-center justify-center rounded-2xl border border-stone-200/60 bg-white/90 p-8 shadow-xl backdrop-blur">
