...
serve(async (req) => {
  ...
  try {
    const { businessData, document, folderName, uploadedFileContent } = await req.json();

    if (!businessData || !document) {
      return new Response(
        JSON.stringify({ error: "Business data and document are required" }),
...
Return ONLY valid JSON, no markdown, no explanation.`;
    ...
    content.sections = content.sections.map((section: any, idx: number) => {
      ...
      if (section.subsections && Array.isArray(section.subsections)) {
        section.subsections = section.subsections.map((subsec: any) => {
    ...
    const totalContentLength = content.sections.reduce((sum: number, s: any) => sum + (s.content?.length || 0), 0);
    console.log(`Document ${content.filename} generated: ${content.sections.length} sections, ${totalContentLength} total chars`);

    return new Response(
      JSON.stringify({ success: true, doc: content }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error) {
    const documentTitle = (await req.json().catch(() => ({})) as any)?.document?.title || "Unknown";
    ...
    } else if (error.message.includes("timeout") || error.message.includes("529")) {
