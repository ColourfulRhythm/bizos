        ...
        return new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            vertexShader: `
              ...
              void main(){
                vec3 updatePosition=(rotateMatrixX(radians(90.))*vec4(position,1.)).xyz;
                float sin1=sin(radians(updatePosition.x/128.*90.));
                vec3 noisePosition=updatePosition+vec3(0.,0.,time*-30.);
                float noise1=cnoise(noisePosition*0.08);
                float noise2=cnoise(noisePosition*0.06);
                float noise3=cnoise(noisePosition*0.4);
                vec3 lastPosition=updatePosition+vec3(0.,noise1*sin1*8.+noise2*sin1*8.+noise3*(abs(sin1)*2.+.5)+pow(sin1,2.)*40.,0.);
                vPosition=lastPosition;
                gl_Position=projectionMatrix*modelViewMatrix*vec4(lastPosition,1.);
              }
            `,
            fragmentShader: `
