    ...
    >
      ...
      >
        {characters.map((character, index) => (
          ...
          >
            {character.href ? (
              <Link href={character.href} onClick={() => handleClick(character, index)}>
                <motion.span
                  className={cn(
                    "relative w-11 h-11 rounded-full flex items-center justify-center text-xl cursor-pointer block",
            ...
            <AnimatePresence>
              {hoveredIndex === index && character.description && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 4 }}
                  className={cn(
                    "absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap pointer-events-none",
                    theme === "dark"
